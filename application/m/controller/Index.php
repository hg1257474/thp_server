...
class Index
{
    ...
    {
        $product_centers = Db::query("select image_uri,name,id from product_center order by sequence asc,id asc limit 6");
    ...
    public function news_center()
    {
        ...
        $current = array_key_exists('current', $params) ? $params['current'] : 1;
        $total = Db::query("select count(*) from news_center ")[0]["count(*)"];
        $is_last = $total <= $current * 7;
        $list = Db::query("select created_at,description,id,name from news_center order by created_at desc limit " . (string) (($current - 1) * 7) . ",7");
        return view("/list", [
            "name" => ["新闻中心", "NEWS CENTER"], "dir" => "list/",
            "current" => $current,
            "list" => $list, "is_last" => $is_last,
            "pages" => range(1, floor(($total - 1) / 7) + 1)
        ]);
    }
    public function honorary_qualification()
    {
        ...
        $is_last = $total <= $current * 7;
        $list = Db::query("select created_at,description,id,name from honorary_qualification order by created_at desc limit " . (string) (($current - 1) * 7) . ",7");
        return view("/list", [
    ...
    public function general_knowledge_encyclopedia()
    {
        ...
        $is_last = $total <= $current * 7;
        $list = Db::query("select created_at,description,id,name from general_knowledge_encyclopedia order by created_at desc limit " . (string) (($current - 1) * 7) . ",7");
        return view("/list", [
